...
async function getStockLeaderboard() {
  const response = await fetch('http://localhost:8080/api/get_top_stocks_leaderboard', {
    cache: 'no-store',
  });
  if (!response.ok) {
 ...
 */
export default async function StockPriceLeaderboard() {
  const leaderboard: any[] = await getStockLeaderboard(); // Wait for the promise to resolve

  return (
    <div className='md:w-[30%]'>
      <div className='mt-2 rounded-lg bg-slate-300 p-4'>
        <h2 className='text-lg mb-2'>Top stocks today</h2>
