...
const GrowingRectangle = ({ width, finalHeight, duration }) => {
  ...
  function getRandomColor() {
    ...
    const randomVariations = baseColor.map(
      (component) =>
        component +
        Math.floor(Math.random() * variation) * (Math.random() < 0.5 ? -1 : 1)
    );

    // Ensure the color components are within the valid range (0-255)
    const correctedComponents = randomVariations.map((component) =>
